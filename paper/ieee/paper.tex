...
The RGATv2 model was trained on 12,000 labeled samples (100 scenarios $\times$ 120 ticks) with 15\% anomaly injection rate, yielding an effective anomaly rate of approximately 5\% at node level. To address this severe class imbalance, we employed a dual strategy: \textit{(i)} class-weighted oversampling via $WeightedRandomSampler$ to balance each batch to approximately 50:50 anomaly:normal samples, and \textit{(ii)} per-sample $pos\_weight$ scaling in the focal loss function to amplify anomaly gradient signals. Figure~\ref{fig:training_loss} shows the training curves.

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{training_loss}
  \caption{RGATv2 training with class imbalance correction. The model was trained for 28 epochs with AdamW, cosine annealing, weighted oversampling ($pos\_weight = 18.1$), and focal loss ($\gamma = 2.0$).}
...
\begin{itemize}
  \item \textbf{Best validation loss:} 1.465 at epoch~18
  \item \textbf{Anomaly recall:} 72.6\% (up from 0\% without balancing)
  \item \textbf{Precision:} 7.1\% (elevated false positive rate due to aggressive oversampling)
  \item \textbf{F1 score:} 0.127 (meaningful improvement over 0.000 without balancing)
  \item \textbf{Global accuracy:} 40.3\% (reduced from 95.4\% due to increased false positives)
  \item \textbf{Convergence:} Early stopping triggered at epoch~28 (patience~15)
  \item \textbf{Total parameters:} 217,475 (RGATv2 backbone)
\end{itemize}

The class imbalance correction successfully activated the model's anomaly detection capability---recall jumped from effectively 0\% to 72.6\%---but the aggressive double-weighting (oversampling combined with $pos\_weight = 18.1$) overcorrected, causing precision to drop to 7.1\%. This precision-recall trade-off is a well-known challenge in imbalanced learning \cite{lin_focal_2017}. Future work will explore moderated $pos\_weight$ values (2--3 instead of 18) and inference threshold tuning to improve precision. The physics-informed regularization term $\mathcal{L}_{\text{phys}}$ constrains the learned representations to physically plausible regions of the voltage space, preventing extreme anomaly scores on nominal buses.
