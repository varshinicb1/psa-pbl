...
RV College of Engineering, Bengaluru, India\\
\{varshinicb, vedant, sethus, aravindkn\}@rvce.edu.in,
...
The RGATv2 model was trained on 6,000 labeled samples (50 scenarios $\times$ 120 ticks) with 20\% anomaly injection rate and $\pm$25\% load perturbations for stronger anomaly signals. Node-level labels were assigned via z-score thresholding on per-snapshot voltage profiles ($|z| > 1.8$), yielding an effective anomaly rate of 0.58\%. Two key improvements were made over the initial training pipeline: \textit{(i)} decoupling of the $WeightedRandomSampler$ from the $pos\_weight$ parameter---the sampler balanced each batch to approximately 50:50 while $pos\_weight$ was set to a mild 2.0 instead of 18.1---eliminating the double-counting that previously caused overcorrection, and \textit{(ii)} inference threshold calibration on the validation set (50-step grid search over $[0.05, 0.95]$) to maximize the F1 score. Figure~\ref{fig:training_loss} shows the training curves.

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{training_loss}
  \caption{RGATv2 training with decoupled class balancing. Model trained for 50 epochs with AdamW, cosine annealing, $WeightedRandomSampler$ + $pos\_weight = 2.0$, label smoothing ($\varepsilon = 0.05$), and focal loss ($\gamma = 2.0$).}
...
\begin{itemize}
  \item \textbf{Best validation loss:} 1.262 at epoch~50
  \item \textbf{Node-level accuracy:} 93.4\% (up from 40.3\% due to reduced false positive rate)
  \item \textbf{Precision:} 9.5\% at optimal threshold $t = 0.60$ (34\% relative improvement over 7.1\%)
  \item \textbf{Recall:} 35.7\% at $t = 0.60$ (precision-optimized operating point)
  \item \textbf{F1 score:} 0.151 (19\% relative improvement over 0.127)
  \item \textbf{Recall at global optimum:} 100\% with precision 7.8\% at $t = 0.05$
  \item \textbf{Total parameters:} 186,307 (RGATv2 backbone)
\end{itemize}

The decoupled balancing strategy successfully resolved the overcorrection issue. By separating the batch-level balancing ($WeightedRandomSampler$) from the per-sample loss weighting ($pos\_weight = 2.0$), the model learned to discriminate anomaly nodes without the aggressive bias that previously caused a flood of false positives. The inference threshold calibration provided an additional lever for navigating the precision-recall trade-off: at $t = 0.60$, the model achieves 9.5\% precision with 35.7\% recall (F1~=~0.151), suitable for applications requiring high-confidence alerts; at $t = 0.05$, recall reaches 100\% with 7.8\% precision (F1~=~0.144), suitable for safety-critical screening. The physics-informed regularization term $\mathcal{L}_{\text{phys}}$ continued to constrain the learned representations to physically plausible regions of the voltage space. The 93.4\% node-level accuracy---up from 40.3\% in the original pipeline---reflects the dramatically reduced false positive rate achieved through proper calibration.
...
\begin{enumerate}
    \item \textbf{GNN precision-recall trade-off:} The RGATv2 achieves 93.4\% node-level accuracy with decoupled class balancing, but node-level precision remains limited (9.5\%) for the challenging per-node localization task on small grids. Future work will explore gradient-based explanation methods (GNNExplainer, integrated gradients) for precise node-level attribution from accurate graph-level predictions.
...
\begin{itemize}
  ...
  \item A physics-constrained RGATv2 GNN achieving 93.4\% node-level accuracy with 9.5\% precision at optimal threshold calibration.
...
The platform demonstrates that a modular digital twin architecture combining physics-based simulation, graph neural networks, and interactive visualization can provide actionable intelligence for real-time grid operations. Future work will address real-time GNN inference integration, multi-grid scalability, and further precision improvements through gradient-based explanation methods for node-level attribution.

% ===================================================================
\section*{Acknowledgment}
% ===================================================================
The authors thank BESCOM Bangalore for providing operational data and domain expertise. This work was conducted as part of the 6th Semester Power System Analysis Project Based Learning (PBL) course at RV College of Engineering, Bengaluru.
